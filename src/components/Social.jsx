  ...
  return (
    <>
      <motion.div
        ...
        viewport={{ once: false }}
      >
        <motion.div variants={textVariant} className="mt-12 bg-[#1c1f2e]">
        ...
        <div className="flex gap-7 flex-wrap justify-center mt-8">
          {socials.map((social, index) => {
            return (
              <a
                className="green-pink-gradient h-20 w-20 rounded-full group flex justify-center items-center cursor-pointer"
                href={social.link}
                target="_blank"
                key={index}
              >
